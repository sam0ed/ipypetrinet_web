...
import './css/widget.css'; // Adjusted path
...
export class PetriModel extends DOMWidgetModel {
  ...
  defaults() {
    const modelDefaults = super.defaults(); // Get defaults from the base stub
    const petriDefaults = {
    ...
    console.log('PetriModel.defaults() setting:', {
        _view_name: petriDefaults._view_name, 
        _view_module: petriDefaults._view_module,
        MODULE_NAME_USED: MODULE_NAME // Log MODULE_NAME directly from version.ts for comparison
    });
    return {
      ...modelDefaults, // Spread base defaults first
      ...petriDefaults   // Then our defaults
    };
  }
...
export class PetriView extends DOMWidgetView {
  ...
  simulationId: any;
  // width: any; // No longer needed as class property if paper is 100%
  // height: any; // No longer needed as class property if paper is 100%

  render() {
    console.log('PetriView.render() called.');
    this.el.classList.add('petri-view-root'); // Add class for styling this.el

    const controlsDiv = document.createElement('div');
    controlsDiv.className = 'petri-controls';
    ...
    controlsDiv.append(dropdown, saveGraph, importJSON, saveIMG, downloadJSON, zoomIn, zoomOut, addPlace,
                       addTrans, addToken, removeToken, setLayout, clearAll, lockModel, simulate, 
                       stopSimulation, reloadSim, addAttrs);

    // Append controls and popups to this.el 
    // Popups are appended to this.el so they are within the view's scope but might need specific CSS for positioning.
    this.el.appendChild(controlsDiv);
    this.el.append(popup, linkPopup, savePopup, uploadPopup, downPopup, condPopup, attrPopup, layoutPopup); // Popups still appended to this.el

    console.log('PetriView.render() - before initWidget()');
    this.initWidget(); 
    console.log('PetriView.render() - after initWidget()');

    if (PetriView.paper && PetriView.paper.el) {
        PetriView.paper.el.id = "paper"; // ID for CSS targeting
        // restrictTranslate is fine here if paper dimensions are controlled by CSS
        PetriView.paper.options.restrictTranslate = function(cellView: joint.dia.CellView) { return cellView.paper!.getArea(); }
        this.el.appendChild(PetriView.paper.el); // Paper appended after controls
        console.log('PetriView.render() - paper.el appended to this.el');
    } else {
        console.error('PetriView.render() - PetriView.paper or paper.el is null/undefined after initWidget!');
    }
    ...
    PetriView.paper.on({
      ...
      'cell:mouseenter': (elementView: joint.dia.CellView, evt?: joint.dia.Event) => {
        if (document.querySelector('#lock')!.textContent == " Lock") {
          var x = "95%"; var y = "30%";
          if (elementView.model.attributes.type == "customTransition") { var x = "98%"; var y = "0%"; }
          if (elementView.model.attributes.type != "pn.Link") {
            // Cast to ElementView if specific ElementView properties are used that are not on CellView
            // addTools itself is on CellView, but what it does internally might matter.
            const elView = elementView as joint.dia.ElementView;
            elView.addTools(
      ...
      'element:mouseleave': (elementView: joint.dia.CellView, evt?: joint.dia.Event) => {
  ...
  private initWidget() {
    console.log('PetriView.initWidget() called.');
    ...
    // Set paper dimensions to 100% to fill its container (#paper div, styled by CSS)
    const paperWidth = '100%';
    const paperHeight = '100%';

    PetriView.paper = new joint.dia.Paper({
      el: document.createElement("div"), // Paper creates its own div
      width: paperWidth,
      height: paperHeight,
      drawGrid: false,
    ...
    // Removed the temporary test place
    // const testPlace = new joint.shapes.pn.Place({
    //     position: { x: 50, y: 50 },
    //     attrs: { '.label': { text: 'Test' } },
    //     tokens: 1
    // });
    // PetriView.graph.addCell(testPlace);
    // console.log('Test place added to graph in initWidget');
    console.log('PetriView.initWidget() finished.'); // Added log
  }
  ...
  private updateGraph() {
    // Temporarily remove the listener to prevent re-entry from this.model.set
    if (this.model) { // Check if model exists
        this.model.off("change:graph", this.updateGraph, this);
    }
    ...
    if (this.model) { // Check if model exists
        this.model.set("graph", res);
        this.model.save_changes(); // This is from the stub, might not do much but harmless
    ...
    for (let childId in xmlPnml.childNodes) {
      ...
      if (child.tagName == "net") {
        ...
        if (child.tagName == "place") {
          ...
          var xPos = 20;
          var yPos = 25;
          var conditions: string[] = []; // Explicitly typed
          var eventattrs: string[] = []; // Explicitly typed
  ...
  private static updateAttrsFrontend(id: string) {
    var list: string[] = []; // Explicitly typed
    $(`#${id}`).empty();
